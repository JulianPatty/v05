/**
Setn Studio Telemetry - Server-side Instrumentation
 *
...
async function initializeOpenTelemetry() {
  try {
    ...
    try {
      ...
      // Remove '.ts' extension to comply with Node.js import rules and avoid TypeScript import path errors.
      telemetryConfig = (await import('./telemetry.config')).default
      logger.debug('Loaded telemetry config from ./telemetry.config')
    } catch (_e) {
      // Fallback to default config if import fails (e.g., config file missing or syntax error)
      logger.warn('Falling back to DEFAULT_TELEMETRY_CONFIG due to import failure', { error: _e })
