...
const CreateTemplateSchema = z.object({
  ...
  state: z.object({
    ...
    parallels: z.record(z.string(),z.any()),
  }),
})
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      logger.warn(`[${requestId}] Invalid query parameters`, { errors: error.message })
      return NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      logger.warn(`[${requestId}] Invalid template data`, { errors: error.message })
      return NextResponse.json(
        { error: 'Invalid template data', details: error.message },
        { status: 400 }
      )
    }
