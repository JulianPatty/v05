...
import type { TraceSpan, ToolCall } from '@/lib/logs/types'
...
export function buildTraceSpans(result: ExecutionResult): {
  ...
  result.logs.forEach((log) => {
    ...
    if (
      ...
      let toolCallsList: any[] | null = null
      ...
      if (toolCallsList && toolCallsList.length > 0) {
        const processedToolCalls = toolCallsList
          .map((tc: any) => {
          ...
          .filter((tc): tc is NonNullable<typeof tc> => tc !== null) // Type-safe filter to remove nulls
        
        span.toolCalls = processedToolCalls as ToolCall[]
