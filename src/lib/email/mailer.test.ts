...
vi.mock('../env', () => ({
  env: {
    ...
    NEXT_PUBLIC_APP_URL: 'https://test.Setn.ai',
  },
}))

vi.mock('../urls/utils', () => ({
  getEmailDomain: vi.fn().mockReturnValue('Setn.ai'),
}))
  ...
  describe('sendEmail', () => {
    it('should send a transactional email successfully', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith({
        from: 'Sim Studio <noreply@Setn.ai>',
    ...
    it('should send a marketing email with unsubscribe headers', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith({
        ...
        headers: {
          'List-Unsubscribe':
            '<https://test.Setn.ai/unsubscribe?token=mock-token-123&email=test%40example.com>',
