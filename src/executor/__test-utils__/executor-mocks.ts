...
import type { SerializedWorkflow, SerializedBlock, SerializedParallel } from '@/serializer/types'
...
export const createMockHandler = (
  handlerName: string,
  options?: {
    canHandleCondition?: (block: SerializedBlock) => boolean
    executeResult?: unknown
  }
) => {
  const defaultCanHandle = (block: SerializedBlock) =>
...
export const setupExecutorCoreMocks = () => {
  ...
  vi.doMock('../loops', () => ({
    LoopManager: vi.fn().mockImplementation(() => ({
      ...
      getLoopIndex: vi.fn().mockImplementation((loopId, _blockId, context) => {
...
export const createWorkflowWithParallel = (distribution?: string[]): SerializedWorkflow => ({
  version: '2.0',
  blocks: [
    {
      id: 'starter',
...
export interface MockContextOptions {
  ...
  parallelExecutions?: Map<string, unknown>
  parallelBlockMapping?: Map<string, unknown>
...
export const createLoopManagerMock = (options?: {
  processLoopIterationsImpl?: (context: MockContextOptions) => Promise<boolean>
  getLoopIndexImpl?: (loopId: string, blockId: string, context: MockContextOptions) => number
}) => ({
  LoopManager: vi.fn().mockImplementation(() => ({
    processLoopIterations: options?.processLoopIterationsImpl || vi.fn().mockResolvedValue(false),
    getLoopIndex:
      options?.getLoopIndexImpl ||
      vi.fn().mockImplementation((loopId, _blockId, context) => {
        return context.loopIterations.get(loopId) || 0
      }),
  })),
})
...
export const createParallelExecutionState = (options?: {
  ...
  distributionItems?: string[] | Record<string, string> | null
  ...
  executionResults: options?.executionResults ?? new Map<string, string>(),
...
export const createParallelManagerMock = (options?: {
  maxChecks?: number
  processParallelIterationsImpl?: (context: MockContextOptions) => Promise<void>
}) => ({
  ParallelManager: vi.fn().mockImplementation(() => {
      ...
      processParallelIterations:
        ...
        vi.fn().mockImplementation(async (context) => {
          for (const [parallelId, parallel] of Object.entries(context.workflow?.parallels || {})) {
            ...
            const parallelNodes = (parallel as SerializedParallel).nodes || []
...
export const createFunctionBlockHandler = vi.fn().mockImplementation(() => ({
  canHandle: (block: SerializedBlock) => block.metadata?.id === 'function',
  execute: vi.fn().mockImplementation(async (_block, inputs) => {
    return {
...
export const createParallelBlockHandler = vi.fn().mockImplementation(() => {
  return {
    canHandle: (block: SerializedBlock) => block.metadata?.id === 'parallel',
    execute: vi.fn().mockImplementation(async (block, _inputs, context) => {
      ...
      if (!parallelState) {
        ...
        const connections =
          context.workflow?.connections.filter(
            (conn: { source: string, sourceHandle: string }) =>
      ...
      if (allCompleted) {
        ...
        const endConnections =
          context.workflow?.connections.filter(
            (conn: { source: string, sourceHandle: string }) => conn.source === parallelId && conn.sourceHandle === 'parallel-end-source'
          ) || []
 ...
 */
export const createParallelInputResolver = (distributionData: unknown) => ({
  InputResolver: vi.fn().mockImplementation(() => ({
...
export const createWorkflowWithParallelArray = (
  items: unknown[] = ['apple', 'banana', 'cherry']
): SerializedWorkflow => ({
  version: '2.0',
  blocks: [
    {
...
export const createWorkflowWithParallelObject = (
  items: Record<string, unknown> = { first: 'alpha', second: 'beta', third: 'gamma' }
): SerializedWorkflow => ({
  version: '2.0',
  blocks: [
    {
